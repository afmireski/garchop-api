...
func (r *SupabasePriceRepository) FindCurrentPrice(pokemonId string) (*models.PriceModel, error) {
	...
	err := r.client.DB.From("prices").Select("*", "pokemons (*, stocks (*))").OrderBy("created_at", "desc").Limit(1).Single().Eq("pokemon_id", pokemonId).Is("deleted_at", "null").Execute(&supabaseData); if err != nil {
